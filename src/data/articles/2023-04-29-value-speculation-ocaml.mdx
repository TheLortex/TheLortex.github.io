...
We can already see that the CPU is cramming multiple instructions per cycles.
...
let rec seum (previous : int) (accu : int) (cell : int list) =
  ...
  | head::tail ->
    ...
    (* equivalent to:
    let delta = current - previous in
    let prediction = current + delta in *)
...
I was not satisfied by this mere 30% improvement. What if the loop was unrolled, so that the CPU has more freedom to move instructions around ?
