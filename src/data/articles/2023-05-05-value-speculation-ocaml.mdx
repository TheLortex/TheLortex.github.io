...
- An `int list` is a pointer to an OCaml block allocated on the heap. Due to alignment, it always end by a zero bit.
...
For example, the addition is implemented as `Int.add a b = a + b - 1`. The consequence is that adding two raw numbers together using the OCaml addition would subtract one from the result to account for the expected tags. 
